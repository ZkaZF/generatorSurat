...
export default function SuratPermohonanBeasiswaPreview({ formData }: Props) {
  ...
  return (
    <div style={PS.page}>
      ...
      <div style={PS.section()}>
        <div>Kepada Yth.</div>
        <div>Tim Seleksi / Panitia Beasiswa</div>
        <div style={PS.bold}>{orDash(lembagaBeasiswa as string)}</div>
        <div>di Tempat</div>
      </div>
      <div style={{ display: 'flex', gap: '12px', marginBottom: '10px' }}>
        <span style={PS.bold}>Perihal</span><span>:</span>
        <span>Permohonan {orDash(namaBeasiswa as string)}</span>
      </div>
      ...
      <div style={PS.dataGrid()}>
        ...
        {ipk && <><span>IPK</span><span>:</span><span>{ipk as string}</span></>}
      </div>
      <div style={PS.p()}>Dengan ini mengajukan permohonan <strong>{orDash(namaBeasiswa as string)}</strong>.</div>
      {motivasi && <div style={PS.p()}>{motivasi as string}</div>}
      <div style={PS.p()}>Demikian surat permohonan ini saya buat. Atas perhatian dan pertimbangan yang diberikan, saya ucapkan terima kasih.</div>
